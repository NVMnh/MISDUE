...
where Cust_phone not like '09[0,3]%'
and Cust_phone not like '089%'
and Cust_phone not like '012[0,1,2,6,8]%'
...
where Cust_name like N'_[H, T, A, Á, À, Ã, Ê]%'

----cau 7
select t_date from transactions
where YEAR(t_date) = 2016 and month(t_date) > 9

----cau 8
select * from transactions
where DATEPART(quarter, t_date) = 3 and DATEPART(year, t_date) = 2016
...
----cau 10
select cust_id, Cust_ad from customer
where Cust_ad like N'%Ngũ Hành Sơn%Đà Nẵng'

----cau 11
select * from transactions where t_time not between '08:00' and '12:00'
and t_time not between '13:30' and '17:30' and t_type = 1

----cau 12
select t_id, t_time, t_type from transactions
where t_time between '0:00' and '03:00' and t_type = 0

----cau 13
select t_id, t_date from transactions
where year(t_date) = 2017 and t_type = 1

----cau 14
Select * From transactions inner join account on transactions.ac_no = account.Ac_no
Where ac_balance < 0
...
----cau 16
select t_id, t_amount, t_type from transactions
...
----cau 19
select cust_id, cust_name from customer
where Cust_name not like '[N, T]%'

----cau 20
Select Cust_name From customer
Where Cust_name like N'%[a, á, à, ả, ã, ạ, u, ú, ù, ủ, ũ, ụ, i, í, ì, ỉ, ĩ, ị]__'
...
----cau 23
Select Cust_id, Cust_name from customer
Where LTRIM(right(Cust_name, CHARINDEX(' ',REVERSE(Cust_name) , 1)-1)) like N'_[u, ũ, a, á, à, ả, ạ, ã]%'
